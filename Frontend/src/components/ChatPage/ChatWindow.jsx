...
const ChatWindow = ({ token, userProfile }) => {
  ...
  // 1. Tạo session khi component mount
  useEffect(() => {
    const initSession = async () => {
      // Chỉ gọi khi có token để tránh lỗi 401
      if (!token) return;

      try {
        const res = await fetch('http://localhost:3001/api/chat/session', {
          method: 'POST',
          headers: { 
  ...
  // Hiển thị loading đồng nhất với phong cách tối giản
  if (isInitializing || !token) {
    return (
      <div className="h-screen flex items-center justify-center bg-white">
        <div className="flex flex-col items-center gap-3">
          <div className="w-8 h-8 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin"></div>
          <p className="text-slate-400 text-sm font-medium">Đang kết nối FSA AI...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-white relative font-sans text-slate-900">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 flex flex-col items-center">
        {messages.length === 0 ? (
          ...
          <div className="w-full max-w-4xl space-y-8 pb-10">
            {messages.map((m) => (
              <div key={m.id} className="flex gap-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                <div className="flex-1 space-y-2 min-w-0">
                  ...
                  <div className="prose prose-slate max-w-none text-slate-700 leading-relaxed text-base prose-headings:font-bold prose-h3:text-lg prose-h3:mt-4 prose-p:mb-3 prose-strong:text-indigo-600 prose-ul:list-disc prose-ul:pl-5 prose-li:mb-1 prose-hr:my-6">
                    <ReactMarkdown 
            ...
            {isLoading && (
               <div className="flex gap-2 p-4 text-slate-400 italic text-sm animate-pulse">
                 FSA AI đang suy nghĩ...
               </div>
            )}
          </div>
        )}
      ...
      {/* INPUT AREA - GIỮ NGUYÊN THIẾT KẾ */}
            ...
            <div className="flex items-center gap-1">
              ...
              {(input || "").trim() !== "" && (
                <button 
                  ...
                  className="p-2.5 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 transition-all animate-in zoom-in shadow-md"
                >
