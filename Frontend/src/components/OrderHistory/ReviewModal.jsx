...
const ReviewModal = ({ order, product, onClose }) => {
  const [rating, setRating] = useState(5);
  const [comment, setComment] = useState('');
  const [selectedFiles, setSelectedFiles] = useState([]);
  const [previews, setPreviews] = useState([]);
  const { postReview, submitting } = useReview(product?.product_id);
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
      <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200">
        <div className="p-6 border-b border-slate-100 flex justify-between items-center">
          <h3 className="font-black text-slate-800 uppercase tracking-tight text-lg">Đánh giá sản phẩm</h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto max-h-[80vh]">
          ...
          <textarea
            className="w-full rounded-2xl border border-slate-200 p-4 text-sm focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50 min-h-[100px] transition-all"
          ...
          >
            {submitting ? 'ĐANG TẢI LÊN...' : 'GỬI ĐÁNH GIÁ NGAY'}
          </button>
        </form>
      </div>
    </div>
