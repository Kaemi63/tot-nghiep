...
/** Trạng thái trống (Dùng cho Giỏ hàng trống, Yêu thích trống...) */
  ...
  <div className="flex flex-col items-center justify-center py-24 text-center">
    ...
    {action && (
      <button 
        ...
        className="mt-6 px-6 py-2.5 rounded-xl bg-indigo-600 text-white font-semibold text-sm hover:bg-indigo-700 transition-colors"
      >
        {action.label}
      </button>
    )}
...
export const Stars = ({ rating = 5 }) => (
  <div className="flex gap-0.5">
    {[1, 2, 3, 4, 5].map((s) => (
      <svg 
        key={s} 
        className={`w-4 h-4 ${s <= rating ? 'text-amber-400 fill-current' : 'text-slate-200'}`} 
        viewBox="0 0 20 20"
      >
